<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:62:"F:\www\pyg\public/../application/home\view\shop_car\index.html";i:1569565538;s:51:"F:\www\pyg\application\home\view\Common\layout.html";i:1570882765;}*/ ?>
<!DOCTYPE html>
<html>
...
<div class="cart py-container">
	<!--All goods-->
	<div class="allgoods">
		<h4>全部商品<span><?php echo count($goodsinfo); ?></span></h4>
		<div class="cart-main">
			...
			<div class="cart-item-list">
				...
				<div class="cart-body">
					<?php if(is_array($goodsinfo) || $goodsinfo instanceof \think\Collection || $goodsinfo instanceof \think\Paginator): $i = 0; $__LIST__ = $goodsinfo;if( count($__LIST__)==0 ) : echo "你的购物车空空如也" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
					<div class="cart-list" carid="<?php echo $v['car_id']; ?>" number="<?php echo $v['number']; ?>">
						<ul class="goods-list yui3-g">
							<li class="yui3-u-1-24">
								<input type="checkbox" class="row_check"  value="<?php echo $v['is_selected']; ?>" <?php if($v['is_selected'] == '1'): ?>checked='checked'<?php endif; ?>/>
							</li>
							...
							<li class="yui3-u-5-24">
								<div class="item-txt"><?php echo (isset($v['value_names']) && ($v['value_names'] !== '')?$v['value_names']:''); ?></div>
							</li>
							<li class="yui3-u-1-8"><span class="price"><?php echo (isset($v['price']) && ($v['price'] !== '')?$v['price']:'v.goods_price'); ?></span></li>
							<li class="yui3-u-1-8">
							...
							<li class="yui3-u-1-8"><span class="sum"><?php if(isset($v['price'])): ?><?php echo $v['price']*$v['number']; else: ?><?php echo $v['goods_price']*$v['number']; endif; ?></span>
							</li>
							<li class="yui3-u-1-8">
					...
					<?php endforeach; endif; else: echo "你的购物车空空如也" ;endif; ?>
				</div>
			</div>
		</div>
		<div class="cart-tool">
			...
			<div class="money-box">
				...
				<div class="sumbtn">
					<a class="sum-btn" href="javascript:;" id='settlement'>结算</a>
				</div>
			</div>
		</div>
...
<script type="text/template" id="tbar-cart-item-template">
		<div class="tbar-cart-item" >
			...
			<div class="jtc-item-goods">
				...
				<a href="javascript:void(0)" class="p-del J-del">删除</a>
			</div>
		</div>
	</script>
<!--侧栏面板结束-->
<script type="text/javascript">
	$(function () {
		...
		var change_num = function (number, element) {
			...
			var car_id = $(element).closest('.cart-list').attr('carid');
			$.post("http://www.pyg.com/home/shop_car/modify",
				{ 'number': number, 'car_id': car_id },
				function (data, textStatus, jqXHR) {
					if (data.code != 200) {
						console.log(data);
						layer.msg('修改失败', { icon: 5 });
					} else {
						$(element).closest('.cart-list').attr('number', number);
					}
				},
				"json"
			);
		...
		$('.check_all').change(function () {
			...
			editstatus({'id':'*','is_selected':Number(status)})
		});
		...
		$('.row_check').change(function () {
			...
			let obj = {
				'id':$(this).closest('.cart-list').attr('carid'),
				'is_selected':status
			}
			editstatus(obj);	
		...
		$('.current_number').change(function () {
			...
			if (!/^\d+$/.test(number) || number == 0) {
				$(this).val($(this).closest('.cart-list').attr('number'));
		...
		$('.delete').click(function () {
			//移除当前行记录
			var car_id = $(this).closest('.cart-list').attr('carid');
			$(this).closest('.cart-list').remove();
			//$(this).closest('ul').parent().remove();
			//计算选中数量和金额
			$.ajax({
				type: "delete",
				url: "http://www.pyg.com/home/shop_car/del/"+car_id,
				data:{'id':car_id},
				dataType: "json",
		...
		$('.sum-btn').click(function () {
			...
			location.href = "http://www.pyg.com/order/settlement";
		});
		...
		function editstatus(data){
			$.ajax({
				type: "POST",
				dataType: "json",
				url: "http://www.pyg.com/home/shop_car/editstatus",
				data: data,
				success: function (response) {
